...
<img src="https://latex.codecogs.com/gif.latex?J(\theta)=\frac{1}{2m}[\sum_{i=1}^{m}h_\theta(x^{(i)}-y^{(i)})^{2}&plus;\lambda&space;\sum_{j=1}^{n}\theta&space;_{j}^{2}]" title="J(\theta)=\frac{1}{2m}[\sum_{i=1}^{m}h_\theta(x^{(i)}-y^{(i)})^{2}+\lambda \sum_{j=1}^{n}\theta _{j}^{2}]" />
